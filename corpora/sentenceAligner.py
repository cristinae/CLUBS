...
# Authors: Liling Tan
#          Cristina España-Bonet (modifications to be used within the Leibniz project)
...
def alignLists(src, trg, mean=parameters.mean, 
         variance=parameters.variance, penalty= parameters.penalty):

    aligned_output = []
    src = per_paragraph(src, delimiter='#')
    trg = per_paragraph(trg, delimiter='#')
    for s,t in izip(src, trg):
        for sentence_pair in align(s, t, mean, variance, penalty):
            #print sentence_pair
            sentence_pair = sentence_pair.replace("~~"," ")
            aligned_output.append(sentence_pair)
